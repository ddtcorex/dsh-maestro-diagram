# Contributing to dsh-maestro-diagram

Thank you for contributing to **dsh-maestro-diagram** (`@ddtcorex/dsh-maestro-diagram`) — the Maestro Diagram Studio host plugin that exposes `mermaid_verify` and `mermaid_drift` as deterministic Cordis tools, paired with the `diagram-studio` skill in `maestro-skills`.

## Getting Started

1. **Fork and clone** `github.com/ddtcorex/dsh-maestro-diagram`.
...
3. Build the Host-only plugin (TypeScript → `lib/`):
...
4. Open the project in your editor. Key layout:
   ...
   ```
   src/host/          # plugin source (rootDir: src/host → lib/index.js flat)
   tests/             # vitest specs (mermaid_verify + mermaid_drift)
   cordis.patch.yml   # Host-only row: inject ['sessions','tools']
   ```

   `lib/index.js` **must** be flat — `test -f lib/index.js` must succeed (not `lib/host/index.js`), otherwise `dsh web` boot fails with `ERR_MODULE_NOT_FOUND`.
...
- `fix/<topic>` — bug fixes
- `feat/<topic>` — new features (new diagram type, editor, drift rule)
...
- **Scope:** optional, without the `dsh-maestro-` prefix — e.g. `feat(diagram):`, `fix(drift):`, `docs(readme):`
...
pnpm test        # vitest run — 8/8 (5 verify + 3 drift)
pnpm build       # tsc — ensures lib/index.js is not stale
test -f lib/index.js  # must be 0 — flat output required
```

Additional checks when relevant:

```bash
# Verify Mermaid blocks in docs (CLI fallback without plugin)
node ../maestro-skills/skills/diagram-studio/scripts/verify-mermaid.mjs docs/architecture.md
...
- Security vulnerabilities: use GitHub's private advisory reporting at `https://github.com/ddtcorex/dsh-maestro-diagram/security/advisories` — do not file a public issue.
